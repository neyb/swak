...
internal object PathPatternCompiler {
    ...
    fun compile(pathPattern: String): CompileResult {
        try {
            return PathPatternCompiler.tryToCompile(pathPattern)
    ...
    private fun tryToCompile(pathPattern: String): CompileResult {
        PathPatternCompiler.checkNoStickenGroups(pathPattern)
        val groupNames = mutableListOf<String>()
        val replace = "^" + pathPattern.replace(PathPatternCompiler.groupRegex) { matchResult ->
            val groupInfo = matchResult.groups[1]!!
            groupNames += groupInfo.value.substringBefore(':')
            val content = PathPatternCompiler.getContentPattern(groupInfo, pathPattern)
            """($content)"""
        }
    ...
    private fun getContentPattern(groupInfo: MatchGroup, pathPattern: String): String {
        ...
        else PathPatternCompiler.defaultGroupPattern(groupInfo, pathPattern)
    }

    private fun defaultGroupPattern(group: MatchGroup, pathPattern: String): String {
        val separator = PathPatternCompiler.getSeparator(group, pathPattern)
